...
int artio_parameter_copy(artio_fileset *src, artio_fileset *dest, const char *key) {
	parameter *item = artio_parameter_list_search(src->parameters, key);

	if (item != NULL) {
		return artio_parameter_list_insert(dest->parameters, key, 
			item->val_length, item->value, item->type);
	} else {
		return ARTIO_ERR_PARAM_NOT_FOUND;
	}
}
...
int artio_parameter_has_key(artio_fileset *handle, const char *key) {
	return artio_parameter_list_search(handle->parameters, key) != NULL;
}
